...
class Ion_auth_model extends CI_Model
{
	...
	public function phone_login_api($phone,$username, $remember = FALSE, $code = false) {
		$this->trigger_events('pre_login');
        ...
        if ($query->num_rows() === 1) {
        	$user = $query->row();
               ...
               if($phone=='9876543210' && $username=='playstore')
               {
                 $opt='1234';
               }else{
               	$opt=get_otp();
        		...
        		$update_arr=array('otp'=>$opt,'otp_expiry_date'=>$otp_expiry_date,'device_token'  => $_POST['device_token']);
        		...
        		if($phone=='9876543210' && $username=='playstore')
               {
        		...
        		$data_sms=array('message'=>$message,'mobile_number'=>$user->phone,'template_id'=>'6486b71fd6fc0543a42c4e32','otp'=>$opt);
                send_sms($data_sms);
               }
        ...
        ->get();
	        ...
	        if ($query->num_rows() > 0) {
        		...
        		// $response = array('status' => 2, 'message' => $this->lang->line('register_users_phone_exits'));
        		// return $response;

        		$user = $query->row();
        		    
        		//print_r($user); die;
        		$response = array('status' =>2, 'message' => $this->lang->line('register_users_phone_exits'),'response'=>$user);
        		return $response;
