 ...
 /**
         ...
         public function otp_verify_post()
    {
        ...
        if ($this->form_validation->run() == true) {
           ...
           $otp  = $this->input->post('otp');
            //print_r($authorized_user); die;
           if($this->input->post('user_type')!=$authorized_user['account']->type)
            {
            $this->response([
                    $this->config->item('rest_status_field_name')  => $this->config->item('rest_status_code_one'),
                    $this->config->item('rest_message_field_name') =>'Sorry ! you can not login here'
                ], REST_Controller::HTTP_OK);
            }
           $authorized_user['account'] = $this->general_model->getOne('users', ['id' => $authorized_user['account']->id,'otp'=>$otp]);
            ...
            if ($authorized_user['account']) {
                ...
                {
                  $noti_arr=array('notification_title'=>'Your Registration','notification_message'=>'Welcome '.$account->first_name.'! Your registration is successfully.','notification_type'=>16,'notification_type_name'=>'Your Registration','user_id'=>$authorized_user['account']->id,'notification_created_on'=>date('Y-m-d H:i:s'));
                ...
                $noti_arr=array('notification_title'=>'Your login','notification_message'=>'Welcome '.$account->first_name.'! You have login successfully .','notification_type'=>21,'notification_type_name'=>'Your Login','user_id'=>$authorized_user['account']->id,'notification_created_on'=>date('Y-m-d H:i:s'));
    ...
    public function add_chat_post() {
        ...
        if ($this->form_validation->run() == true) {
            ...
            if($insert_data) {
                  /*-------------Create Notification------------*/
                  $noti_arr=array('notification_title'=>'chat message','notification_message'=>$message,'notification_type'=>1,'notification_type_name'=>'chat','user_id'=>$user_id,'notification_created_on'=>date('Y-m-d H:i:s'));
                  $this->general_model->insert('notifications',$noti_arr);
                /*--------------------------------------------*/
                $this->response([
                    $this->config->item('rest_status_field_name')   => $this->config->item('rest_status_code_one'),
...
public function update_user_type_post(){
        ...
        if ($this->form_validation->run() == true) {
            ...
            if ($res_in_up) {
                if($this->input->post('user_like_status')=='No')
                {
                  ...
                  {
                  $noti_arr=array('notification_title'=>'Liked Your Profile','notification_message'=>'Hey, someone liked your profile.','notification_type'=>36,'notification_type_name'=>'Liked Your Profile','user_id'=>$res->id,'notification_created_on'=>date('Y-m-d H:i:s'));
                  $this->general_model->insert('notifications',$noti_arr);
                /*==================push notification send=================*/
        ...
        if ($this->form_validation->run() == true) {
            ...
            if (!empty($res)) {
                ...
                'individual_id' =>$this->input->post('user_id'),'view_status'=>'Yes','date'=>date('Y-m-d H:i:s')];
                 ...
                 if($res->notification=='on')
                  {
                  $noti_arr=array('notification_title'=>'Visited Your Profile','notification_message'=>'Hey, someone visited your profile.','notification_type'=>35,'notification_type_name'=>'Visited Your Profile','user_id'=>$res->id,'notification_created_on'=>date('Y-m-d H:i:s'));
                  $this->general_model->insert('notifications',$noti_arr);
                /*==================push notification send=================*/
  ...
  public function update_online_status_post(){
     ...
     public function update_city_post(){
        ...
        $this->form_validation->set_rules('active_city','','required');
        $this->form_validation->set_rules('organization_id','','required');

        if ($this->form_validation->run() == true) {
             $update_data = [
                'active_city' =>$this->input->post('active_city')];
                 $isUpdate = $this->general_model->update('organization', ['user_id' => $authorized_user['account']->id,'id'=>$this->input->post('organization_id')], $update_data);
                 //echo $this->db->last_query(); die;
            if ($isUpdate) {
                $this->response([
                    $this->config->item('rest_status_field_name') => $this->config->item('rest_status_code_one'),
                    $this->config->item('rest_message_field_name') =>'User city update successfully',
                ], REST_Controller::HTTP_OK); 
            } else {
                $this->response([
                    $this->config->item('rest_status_field_name') => $this->config->item('rest_status_code_zero'),
                    $this->config->item('rest_message_field_name') => 'User city not update'
                ], REST_Controller::HTTP_OK); 
            }
        } else {
            $this->response([
    ...
    public function update_online_time_expiry_post()
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            if($isUpdate) {
                ...
                $res_plan = $this->general_model->getOne('user_subscriptions_purchase',array('user_id'=>$authorized_user['account']->id,'plan_active_status'=>'1','payment_status'=>'1'));
               //echo $this->db->last_query(); die;
                if(!empty($res_plan))
                {
                     if($res_plan->plan_expired_status=='Yes')
                     {
                      $plan_plan_expiry="Yes";
                     }else{
                      $plan_plan_expiry="No";
                     }
                }else{
                ...
                $res_org = $this->general_model->getOne('organization',array('user_id'=>$authorized_user['account']->id,'approval_status'=>'1'));
                if(!empty($res_org))
                {
                     $approval_date=$res_org->approval_date;
                     $approval_status=$res_org->approval_status;
                     $approval_desciption_status=$res_org->approval_desciption_status;

                }else{
                    $approval_date="";
                    $approval_status="";
                    $approval_desciption_status="";
                }

                $res_user_chat = $this->general_model->getOne('user_chat_list',array('user_id_from'=>$authorized_user['account']->id,'status'=>'1'));
                if(!empty($res_user_chat))
                {
                     $chat_status='1';
                }else{
                     $chat_status='0';
                }
                $res_noti = $this->party_model->get_notification_count($authorized_user['account']->id);
                ...
                $this->response([
                    ...
                    'plan_plan_expiry'=>$plan_plan_expiry,
                    'approval_date'=>$approval_date,
                    'approval_status'=>$approval_status,
                    'approval_desciption_status'=>$approval_desciption_status,
                    'chat_count'=>$chat_status,
                    'notification_count'=>(string)$res_noti
                ], REST_Controller::HTTP_OK);
            }else{
                $this->response([
                    $this->config->item('rest_status_field_name')  => $this->config->item('rest_status_code_zero'),
