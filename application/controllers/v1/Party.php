...
class Party extends REST_Controller {
       ...
       //print_r($authorized_user['account']); die;
        ...
        if ($this->form_validation->run() == true) {
           ...
           /*------------------------------------------------*/
            ...
            if($insert_data) {
                 /*-------------Create Notification------------*/
                  $noti_arr=array('notification_title'=>'Party Post','notification_message'=>'Congratulations! your party is posted and its under review.','notification_type'=>2,'notification_type_name'=>'Party Post','user_id'=>$authorized_user['account']->id,'notification_created_on'=>date('Y-m-d H:i:s'));
    ...
    public function spical_char_remove($string)
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            if($insert_data) {
                if($this->input->post('type')==1)
                    {
                  /*-------------Create Notification------------*/
                  $noti_arr=array('notification_title'=>'Organization Registration','notification_message'=>'Welcome aboard! Your registration is complete and your profile is under review.','notification_type'=>1,'notification_type_name'=>'Organization Registration','user_id'=>$authorized_user['account']->id,'notification_created_on'=>date('Y-m-d H:i:s'));
                  $this->general_model->insert('notifications',$noti_arr);
                   /*==================push notification send=================*/
                ...
                /*-----------------------------------------------------*/
                  ...
                  $noti_arr=array('notification_title'=>'Individual Registration','notification_message'=>'Congratulations! You have registered successfully. Profile and photos are under review.','notification_type'=>34,'notification_type_name'=>'Individual Registration','user_id'=>$authorized_user['account']->id,'notification_created_on'=>date('Y-m-d H:i:s'));
                  $this->general_model->insert('notifications',$noti_arr);
                   /*==================push notification send=================*/
        ...
        if ($this->form_validation->run() == true) {
            ...
            if($update_data) {
                ...
                /*-----------------------------------------------------*/
                    ...
                    /*-------------Create Notification------------*/
                  $noti_arr=array('notification_title'=>'Individual Profile Updated','notification_message'=>'Your profile and photos are updated,waiting for approval.','notification_type'=>37,'notification_type_name'=>'Individual Profile Updated','user_id'=>$authorized_user['account']->id,'notification_created_on'=>date('Y-m-d H:i:s'));
                  $this->general_model->insert('notifications',$noti_arr);
                   /*==================push notification send=================*/
             ...
             foreach ($data as $key => $value) {
                $party_res = $this->general_model->getOne('party_ongoing_history', ['user_id' => $authorized_user['account']->id,'party_id'=>$value->id]);
                 ...
                 if(!empty($org_res))
                 {
                    ...
                    $data[$key]->organization=$org_res->name;
                 }else{
             ...
             if(empty($res_like_party))
             {
               ...
               if(!empty($res_org_user))
                { 
                  ...
                  $noti_arr=array('notification_title'=>'Party Like','notification_message'=>'Someone like your profile.','notification_type'=>31,'notification_type_name'=>'Party Like','user_id'=>$res->user_id,'notification_created_on'=>date('Y-m-d H:i:s'));
                  $this->general_model->insert('notifications',$noti_arr);
                /*==================push notification send=================*/
            ...
            $res_view_party= $this->general_model->getOne('party_view_history', ['party_id' =>$this->input->post('party_id'),'user_id' => $authorized_user['account']->id]);
             if(empty($res_view_party))
             {
               ...
               if(!empty($res_org_user))
                { 
                  //print_r($res_org_user); die;
                  $noti_arr=array('notification_title'=>'Party View','notification_message'=>'Someone view your party.','notification_type'=>32,'notification_type_name'=>'Party View','user_id'=>$res->user_id,'notification_created_on'=>date('Y-m-d H:i:s'));
                  $this->general_model->insert('notifications',$noti_arr);
                /*==================push notification send=================*/
            ...
            $res_ongoing_party= $this->general_model->getOne('party_ongoing_history', ['party_id' =>$this->input->post('party_id'),'user_id' => $authorized_user['account']->id]);
             if(empty($res_ongoing_party))
             {
               ...
               if(!empty($res_org_user))
                { 
                  //print_r($res_org_user); die;
                  $noti_arr=array('notification_title'=>'Party Joined','notification_message'=>'Someone joined your party.','notification_type'=>33,'notification_type_name'=>'Party Joined','user_id'=>$res->user_id,'notification_created_on'=>date('Y-m-d H:i:s'));
                  $this->general_model->insert('notifications',$noti_arr);
                /*==================push notification send=================*/
            ...
            $res_wish_list= $this->general_model->getOne('party_wish_list', ['party_id' =>$this->input->post('party_id'),'user_id' => $authorized_user['account']->id]);
             if(empty($res_wish_list))
             {
                ...
                $this->general_model->insert('party_wish_list',$insert_data);
                  ...
                  if($authorized_user['account']->notification=='on')
                  {
                  $noti_arr=array('notification_title'=>'Party Wish List Add','notification_message'=>'You have liked the event,added to wishlist.','notification_type'=>35,'notification_type_name'=>'Party Wish List Add','user_id'=>$authorized_user['account']->id,'notification_created_on'=>date('Y-m-d H:i:s'));
                  $this->general_model->insert('notifications',$noti_arr);
                   /*==================push notification send=================*/
