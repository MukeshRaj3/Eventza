...
class Home extends REST_Controller {
        ...
        if (!empty($users)) {
            ...
            foreach ($users as $key => $value) {
                ...
                // echo $this->db->last_query(); die;
                 ...
                 if(!empty($res_user) || !empty($res_user2))
                 {
                   $block_status='1';
                 }else{
                    $block_status='';
    ...
    public function users_search_post() {
        ...
        /* End Check Authentications */
         $this->form_validation->set_rules('keyword','', 'required', array('required' => '%s'));
        if ($this->form_validation->run() == true) {
        $keyword=$this->input->post('keyword');
        $gender=$this->input->post('gender');
       // if($this->form_validation->run() == true) {
        $users = $this->home_model->get_search_users($keyword,$authorized_user['account']->id,$gender);
        if (!empty($users)) {
            foreach ($users as $key => $value) {
               /* $res_org=$this->general_model->getOne('organization',array
                    ('user_id'=>$value->id));
                 ...
                 if(!empty($res_org))
                 {
                   $profile_pic=$res_org->profile_pic;
                 }else{
                   $profile_pic='';
                ...
                $users[$key] = [
                    ...
                    'phone' => $value->phone,
                    //'country_code' => $value->country_code,
                    'profile_picture' => $value->profile_pic ? $value->profile_pic : "",
                    'gender' => $value->gender,
                    'online_status'=>$value->online_status,
                    'privacy_online'=>$value->privacy_online,
                    'like_status'=>$like_status
