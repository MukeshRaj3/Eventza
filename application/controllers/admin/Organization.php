...
class Organization extends Admin_Controller {
  ...
  public function ajax_list() {
    ...
    foreach ($list as $key => $user) {
       ...
       $res = $this->general_model->getOne('users',array('id'=>$user->user_id));
       if(!empty($res))
       {
         $user_phone_num=$res->phone;
       }else{
         $user_phone_num='';
       }
...
$display_status='<select class="form-control" id="display_status'.$user->id.'" onchange="changes_display_status('.$user->id.')">';
                             $selectd1='';
                             $selectd2='';
                            
                            if($user->display_status=='1')
                            {
                              $selectd1='selected';
                            }
                            if($user->display_status=='0')
                            {
                              $selectd2='selected';
                            }
                              ...
                              $display_status .= '<option value="1" '.$selectd1.'>Yes</option>
                              <option value="0" '.$selectd2.' >No</option>
                             
                              </select>';


        $view_detail .= '<p><button type="button" class="ml-1 btn btn-danger btn-sm waves-effect waves-light organization_delete_confirmation" data-toggle="modal" data-target="#organization_deleted" data-id='.$user->id.'><i class="mdi mdi-delete"></i></button></p>';
  ...
  public function edit($id = NULL) {
   ...
   public function delete() {
      $delete = $this->organization_model->delete($this->input->post('organization_id'));
      if ($delete) {
        $response = [
          'status' => 1,
                    ...
                    <table width="100%">
                        <tbody>
                            <tr>
                                ...
                                <td colspan=""><span class=""><img src="'.$organization->profile_pic.' " width="343px" height="161px"></span> </td>
                            </tr>
                            <tr>
                                <td width="200px" class="pr-2"><b>Time Line Pic</b></td>
                                <td colspan=""> <span class=""><img src="'.$organization->timeline_pic.' " width="343px" height="161px"></span> </td>
                            </tr>
                        
                            <tr>
                                <td width="200px" class="pr-2"><b>Name</b></td>
                                <td width="200px">'.$organization->name.'</td>
                            </tr>
                            <tr>
                                <td class="pr-2"><b>Display Name</b></td>
                                <td>'.$organization->display_name.'<span class=""><button type="button" class="ml-1 btn btn-danger btn-sm waves-effect waves-light" data-toggle="modal" data-target="#update_display_name'.$organization->id.'" data-id='.$organization->id.'><i class="fa fa-plus" aria-hidden="true"></i>
                                </button><div id="update_display_name'.$organization->id.'" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
                                    <div class="modal-dialog">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h4 class="modal-title" id="myModalLabel">Update Display Name</h4>
                                                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
                                            </div>
                                            <div class="modal-body">
                                                   <div class="row">
                                            ...
                                            <lable>Display Name</lable>
                                               <input type="text" id="display_name'.$organization->id.'" class="form-control" value="'.$organization->display_name.'">
                                            </div>
                                            </div>
                                            </div>
                                            <div class="modal-footer">
                                                <button type="button" class="btn btn-dark waves-effect" data-dismiss="modal">Cancel</button>
                                                <button type="button" onclick="update_display_name('.$organization->id.')" class="btn btn-primary waves-effect waves-light">Update</button>
                                            </div>
                                        </div><!-- /.modal-content -->
                                    </div>
                                </div></span> </td>
                            ...
                            <tr>
                                ...
                                <td width="200px">'.$organization->description.'</td>
                            </tr>';
                        ...
                        $data .= '<tr>
                            ...
                            <tr>
                                ...
                                <td class="pr-2"><span style="text-align: justify;" class="amenitie">'.$organization->amenitie.'</span> </td>
                            </tr>
                            <tr>
                            ...
                            <tr>
                                ...
                                <td> <span class="latitude">'.$organization->latitude.'</span> </td>
                            </tr>
                            <tr>
                                <td class="pr-2"><b>Longitude</b></td>
                                <td> <span class="longitude">'.$organization->longitude.'</span> </td>
                            </tr>
...
public function pdf_verification_status_update_data()
    ...
    public function changes_approval_image_status()
  {
    ...
    if($update){
      ...
      {
         /*-------------Create Notification------------*/
                  $noti_arr=array('notification_title'=>'Organization Photo Approval','notification_message'=>'Congratulations! Your organization photo is approved.','notification_type'=>11,'notification_type_name'=>'Organization Photo Approval','user_id'=>$res->user_id,'notification_created_on'=>date('Y-m-d H:i:s'));
                  $this->general_model->insert('notifications',$noti_arr);
                /*--------------------------------------------*/
      }else if($update_arr['profile_pic_approval_status']==2){
         /*-------------Create Notification------------*/
                  $noti_arr=array('notification_title'=>'Organization Photo Approval Rejected','notification_message'=>'Oops! Your organization photo is rejected upload another one.','notification_type'=>12,'notification_type_name'=>'Organization Photo Approval Rejected','user_id'=>$res->user_id,'notification_created_on'=>date('Y-m-d H:i:s'));
                  $this->general_model->insert('notifications',$noti_arr);
                /*--------------------------------------------*/
      }
      $response = [
  ...
  public function changes_approval_status()
  {
    ...
    if($update){
      if($update_arr['approval_status']==1)
      {
         /*-------------Create Notification------------*/
                  date_default_timezone_set('Asia/Kolkata');
                  $noti_arr=array('notification_title'=>'Organization Approval','notification_message'=>'Congratulations! your organization is approved.','notification_type'=>13,'notification_type_name'=>'Organization Approval','user_id'=>$res->user_id,'notification_created_on'=>date('Y-m-d H:i:s'));
                  $this->general_model->insert('notifications',$noti_arr);
                /*--------------------------------------------*/
      }
      $response = [
  ...
  public function send_remark()
  {
    ...
    if($res){
         ...
         /*-------------Create Notification------------*/
          date_default_timezone_set('Asia/Kolkata');
          $noti_arr=array('notification_title'=>'Organization - '.$res->name,'notification_message'=>$_POST['description'],'notification_type'=>16,'notification_type_name'=>'Organization Remark','user_id'=>$res->user_id,'notification_created_on'=>date('Y-m-d H:i:s'));
                  $this->general_model->insert('notifications',$noti_arr);
          /*--------------------------------------------*/
          $response = [
              'status' => 1,
...
public function changes_bluetick_status()
  {
   ...
   public function changes_display_status()
  {
     $update_arr=$_POST;
     unset($update_arr['org_id']);
     $update = $this->organization_model->update($_POST['org_id'],$update_arr);
    if($update){
      $response = [
              'status' => 1,
              'message' => 'Display status status change successfully.'
            ];
        }else{
          $response = [
            'status' => 0,
            'message' => 'Display status status not change'
          ];
        }
    echo json_encode($response);
  } 
